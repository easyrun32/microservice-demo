...
if (outputs["aws-region"].value === undefined) {
  console.log(
    'In root directory of terraform outputs.tf /n check if  output "aws-region"{value = var.aws-region} exists'
  );
  console.log("then do terraform apply");
}
...
(async () => {
  ...
  if (fs.existsSync(lockFilePath)) {
    console.error(
      "Lockfile deploy.lock found! Halting... DELETE deploy.lock IN ROOT DIRECTORY to retry"
    );
    process.exit();
  }
